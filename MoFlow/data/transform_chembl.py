...
chembl_atomic_num_list = [5, 6, 7, 8, 9, 14, 15, 16, 17, 34, 35, 53, 0]  # 0 is for virtual node.
max_atoms = 60
n_bonds = 4


def one_hot_chembl(data, out_size=60):
    num_max_id = len(chembl_atomic_num_list)
    assert data.shape[0] == out_size
    b = np.zeros((out_size, num_max_id), dtype=np.float32)
    for i in range(out_size):
        ind = chembl_atomic_num_list.index(data[i])
        b[i, ind] = 1.
    return b


def transform_fn_chembl(data):
    node, adj, label = data
    # convert to one-hot vector
    # node = one_hot(node).astype(np.float32)
    node = one_hot_chembl(node).astype(np.float32)
...
def get_val_ids(file_path='../data/valid_idx_chembl.json'):
